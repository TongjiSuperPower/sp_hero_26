...
//功率限制
#ifdef DT7
  //回滞电压区间
  if (low_vol_flag == 0 && super_cap.voltage >= 6) {
    low_vol_flag = 1;
  }
  if (low_vol_flag == 1 && super_cap.voltage < 3) {
    low_vol_flag = 0;
  }
  ...
  if (!remote.keys.c) {
    ...
    if (super_cap.voltage >= 18 && low_vol_flag == 1) {
      infact_Pmax = CAP_MAX_POWER;
    }
    else if (super_cap.voltage >= 7.5 && low_vol_flag == 1) {
      //线性递减
      infact_Pmax = pm02.robot_status.chassis_power_limit + super_cap.voltage / 18 * 70;
    }
    else {
      infact_Pmax = pm02.robot_status.chassis_power_limit;
    }
