  ...
  Wheel_Torque * Data1, wheel_speed * Data2, wheel_speed * Data3, float P_max)
{
  ...
  //电机的实际速度
  float wheel_speed[4] = {Data2->lf, Data2->lb, Data2->rf, Data2->rb};
  //电机的目标速度
  float wheel_target_speed[4] = {Data3->lf, Data3->lb, Data3->rf, Data3->rb};
  ...
  wheel_is_alive[1] = wheel_lb.is_alive(stamp_ms);
  wheel_is_alive[2] = wheel_rf.is_alive(stamp_ms);
  wheel_is_alive[3] = wheel_rb.is_alive(stamp_ms);
  ...
  for (int i = 0; i < 4; i++) {
    if (wheel_is_alive[i]) {
      //用最大damping率计算各轮衰减后目标速度
      //最大damping反映底盘的最大加速能力，若使用小damping则会加速过慢，影响底盘响应速度；使用平均值则会出现领先轮加速能力被浪费，慢轮仍跟不上
      //最大damping能够最大程度利用底盘的加速能力
