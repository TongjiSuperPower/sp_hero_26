...
constexpr float MAX_WHEEL_TORQUE = 5.859f;       // 轮子电机最大扭矩，单位N.m
constexpr float MAX_FRIC_MOTOR_TORQUE = 5.859f;  //摩擦轮电机最大扭矩，单位N.m
constexpr float TRIGGER_MAX_TORQUE = 1.8f;       //拨弹轮电机最大扭矩，单位N.m

#ifdef HERO_DOG
inline sp::PID yaw_pos_pid(T_CONTROL, 10.0f, 0.0f, 0.0f, 7, 3, 1.0f, true, false);
inline sp::PID yaw_speed_pid(
  T_CONTROL, 1.0f, 0.0f, 0.0f, MAX_4310_TORQUE, MAX_4310_TORQUE / 3.0f, 1.0f, false, false);
...
inline sp::PID ChassisFollow_wz_pid(T_CONTROL, 5.0f, 0.0f, 0.0f, 3.0f, 1.0f, 1.0f, true, false);
